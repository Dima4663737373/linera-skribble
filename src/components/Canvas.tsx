...
export const Canvas = forwardRef<CanvasHandle, CanvasProps>(function Canvas(
  ...
  useEffect(() => {
    ...
    try {
      ...
      wsRef.current = ws;
      ws.onopen = () => {
        try { ws?.send(JSON.stringify({ type: 'join', roomId, clientId })); } catch {}
      };
      ws.onmessage = (evt) => {
        try {
          ...
          if (msg.type === 'draw') {
            ...
            // Do not save history on every network segment to avoid spam
            ...
            // When remote drawer ends stroke, snapshot local canvas as well
            saveToHistory();
          }
        } catch {}
      };
