...
use doodle_game::{Operation, DoodleGameAbi, Player, GameState, ChatMessage};
use linera_sdk::{
...
impl Contract for DoodleGameContract {
    ...
    async fn execute_operation(&mut self, operation: Operation) -> () {
        match operation {
            ...
            Operation::EndMatch => {
                if let Some(room) = self.state.room.get().clone() {
                    ...
                    if room.host_chain_id == current_chain.to_string() {
                        ...
                        // 1. FIRST: Send room deletion message to ALL players (clears their state)
                        eprintln!("[END_MATCH] Sending RoomDeleted to {} players", room.players.len());
                        ...
                        // 4. FINALLY: Delete host's own room state (last step!)
                        self.state.room.set(None);
                        self.state.current_word.set(None);
            ...
            Operation::LeaveRoom => {
                if let Some(room) = self.state.room.get().clone() {
                    let timestamp = self.runtime.system_time().micros().to_string();
                    let current_chain_str = self.runtime.chain_id().to_string();
                    if room.host_chain_id == current_chain_str {
                        let room_host = room.host_chain_id.clone();
                        let player_count = room.players.len();
                        eprintln!("[LEAVE_ROOM] Host leaving room. Host: {}, Players: {}", room_host, player_count);
                        eprintln!("[LEAVE_ROOM] Sending RoomDeleted to {} players", room.players.len());
                        for player in &room.players {
                            if let Ok(chain_id) = player.chain_id.parse() {
                                let deletion_message = doodle_game::CrossChainMessage::RoomDeleted {
                                    timestamp: timestamp.clone(),
                                ...
                                self.runtime.send_message(chain_id, deletion_message);
                                eprintln!("[LEAVE_ROOM] RoomDeleted sent to chain {:?} ({})", chain_id, player.name);
                            }
                        }
                        self.runtime.emit(format!("game_events_{}", room.room_id).into(), &doodle_game::DoodleEvent::MatchEnded { 
                            timestamp: timestamp.clone()
                        ...
                        self.state.room.set(None);
                        self.state.current_word.set(None);
                        eprintln!("[LEAVE_ROOM] Host cleared local state and unsubscribed from all players");
                    } else {
                        eprintln!("[LEAVE_ROOM] Player leaving room");
                        if let Ok(host_chain) = room.host_chain_id.parse() {
                            let player_name_opt = room.players.iter()
                                .find(|p| p.chain_id == current_chain_str)
                                .map(|p| p.name.clone());
                            let leave_msg = doodle_game::CrossChainMessage::PlayerLeft {
                                player_chain_id: self.runtime.chain_id(),
                                player_name: player_name_opt,
                                timestamp: timestamp.clone(),
                            };
                            self.runtime.send_message(host_chain, leave_msg);
                            eprintln!("[LEAVE_ROOM] Sent PlayerLeft to host chain {}", room.host_chain_id);
                        }
                        if let Ok(host_chain) = room.host_chain_id.parse() {
                            let app_id = self.runtime.application_id().forget_abi();
                            let stream = StreamName::from(format!("game_events_{}", room.room_id));
                            self.runtime.unsubscribe_from_events(host_chain, app_id, stream);
                            eprintln!("[LEAVE_ROOM] Unsubscribed from host game_events stream");
                        }
                        self.state.room.set(None);
                        self.state.current_word.set(None);
                        self.state.subscribed_to_host.set(None);
                        eprintln!("[LEAVE_ROOM] Player cleared local state completely");
                    }
                } else {
                    eprintln!("[LEAVE_ROOM] ERROR: No active room to leave");
                }
            }
        }
    }
    ...
    async fn execute_message(&mut self, message: Self::Message) {
        match message {
            ...
            doodle_game::CrossChainMessage::RoomDeleted { timestamp } => {
